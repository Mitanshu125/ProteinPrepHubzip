...
function computeStreak() {
  ...
  const d = new Date()
  while (true) {
    if (log.includes(d.toDateString())) { count++; d.setDate(d.getDate() - 1) }
    else break
  }
  return count
}
...
function ProteinTracker() {
  ...
  const reset = () => {
    setConsumed(0); setMeals([]); setWater(0)
      ...
      <div className="tracker-layout">
        ...
        <div className="tracker-sidebar">
          <div className="streak-card">
            ...
            <p className="streak-msg">{streak} day streak! Keep going 🔥</p>
          </div>
          ...
          <div className="sources-card">
            ...
            <div className="source-row">
              <div className="source-header-row">
                ...
                <span className="source-amount">
                  {meals.reduce((s, m) => s + (m.carbs || 0), 0)}g / 200g
                </span>
              </div>
              <div className="source-bar-track">
                <div className="source-bar-fill" style={{ width: `${Math.min((meals.reduce((s, m) => s + (m.carbs || 0), 0) / 200) * 100, 100)}%` }} />
              </div>
            </div>
            ...
            <div className="source-row">
              <div className="source-header-row">
                ...
                <span className="source-amount">
                  {meals.reduce((s, m) => s + (m.fats || 0), 0)}g / 60g
                </span>
              </div>
              <div className="source-bar-track">
                <div className="source-bar-fill" style={{ width: `${Math.min((meals.reduce((s, m) => s + (m.fats || 0), 0) / 60) * 100, 100)}%` }} />
              </div>
            </div>
